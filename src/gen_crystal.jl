...
μf (float): Post-scattering attenuation coefficent, in cm^-1.
...
function a_grid_calc(
    ...
    @inbounds for I in idx
        ...
        df = kf / norm(kf)
        atten_grid[I], acc_pts = atten_calc(df, v1s, e2s, e3s, coords, len_i, p_f, det_f, λs, path_lengths, n_faces, n_mc, μi, μf[I[1]])
        acc_rate += acc_pts
    end
